...
import joblib                                   # <-- add
feature_scaler = joblib.load('feature_scaler_stft_vctk.pkl') # file you dumped in training
label_scaler   = joblib.load('label_scaler_stft_vctk.pkl')
...
model = load_model('Model_TDDLSTM_VCTK.keras')
...
def reconstruct_audio(lowaudio, sample_rate, highAudio, mirrored_low_audio):
    ...
    pred_windows = model.predict(X_windows_norm, verbose=0)  # Expected shape: (n_windows, window_size, 513)
...
def processfile(file_path):
    ...
    global total_audio_sec, total_wall_e2e, total_wall_lpf
    print(f"Processing file: {file_path}")
    try:
        fs = 16000

        # --- START end-to-end timer (includes load, STFTs, predict, save)
        ...
        save_audio(reconstructed_audio, file_path, r'C:\Users\HPC3\PycharmProjects\mert\STFT_TDDLSTM\Results_VCTK', fs)

        # --- STOP end-to-end timer and accumulate BEFORE metrics
        t_e2e_end = time.perf_counter()
        e2e_wall = t_e2e_end - t_e2e_start

        tot += 1
        total_wall_e2e += e2e_wall
        total_wall_lpf += lpf_wall
        total_audio_sec += (minlen / fs)

        # --- (Optional) metrics AFTER timing so they don't affect RTF
        try:
...
# --- RTF accumulators
total_audio_sec = 0.0     # sum of processed audio length (s)
total_wall_e2e  = 0.0     # end-to-end wall time (load → save)
total_wall_lpf  = 0.0     # LPF-only wall time
...
# --- limit for quick RTF measurement
MAX_FILES = 100
processed = 0

for root, dirs, files in os.walk(test_folder):
    files.sort()  # deterministic order (optional but nice)
    for file in files:
...
print(f"Processed {processed} files (capped at {MAX_FILES}).")

if tot > 0 and total_audio_sec > 0:
    rtf_incl_lpf = total_wall_e2e / total_audio_sec
    rtf_no_lpf   = (total_wall_e2e - total_wall_lpf) / total_audio_sec

    print(f'Processed {tot} files | total audio = {total_audio_sec:.1f} s')
    print(f'RTF (E2E incl. LPF): {rtf_incl_lpf:.3f}  -> {1.0/rtf_incl_lpf:.2f}x real-time')
    print(f'RTF (E2E minus LPF): {rtf_no_lpf:.3f}    -> {1.0/rtf_no_lpf:.2f}x real-time')

    # Metrics summary (optional)
    if Pesavg > 0 or stoiavg > 0:
        print('Averages')
        print(f'Average STOI: {stoiavg/tot:.4f}, Average PESQ: {Pesavg/tot:.4f}')
